...
$announce = events::getEvents($announceID,$isImg);
$announce['evtDate'] = niceTime($announce['evtDate']);
$delImg = 0;

if (isset($_POST['submit'])) {
	if (events::validateEvents($_POST)) {	
	$delImg = $_POST['delImg'];			
        events::editEvents($_POST,$announceID);
		/*if ($_FILES['picture']['name'] != "")
		{
			if (events::validateImage($announce_id, $_FILES)) {
				$isUpload = events::attachPhoto($announce_id, $_FILES['picture']);
			}
		}*/
        if (isset($_POST['img_url'])) {
			if($isImg){
            events::editPhotoUrl($announceID, $_POST['img_url']);
			}else{
			events::attachPhotoUrl($announceID, $_POST['img_url']);
			}
			$delImg = false;	
        }
		if($delImg){
		events::deletePhoto($announceID);	
		}
		successMessage("Announcement edited!", 'viewAnnounce.php?announce_id='.$announceID);
		die(0);	
} 
}else if(isset($_POST['delete'])) {
		events::deleteEvents($announceID);
		successMessage("Announcement has been deleted.", 'viewAnnounceList.php');
	}
...
<div class="col-md-8"><h2>Edit Announcement</h2>
 <?php if($isImg){ ?>
 <label class='col-md-8' rel='tooltip'>Current Announcement Image:</label><br>
<div id="evtImg" class='well' class='col-md-8'><a id="imgRemove" class="btn btn-danger pull-right"><i class="icon-remove"></i></a><img src="<?php echo htmlentities($announce['image']);?>"></img></div>
 <?php } ?>
<form name="addEvent" class='form-horizontal' enctype="multipart/form-data" method='post' id="formAddEvent" action=""><fieldset>
	<div class='form-group'>
		<label class="col-md-3" for="title">Announcement Name:</label>
		<input type="text" id="title" name="title" class="form-control" maxlength="150" value="<?php echo htmlentities($announce['title']); ?>">
	</div>
	   <div class='form-group'>
      <label class='col-md-3' rel='tooltip'>Date of Announcement</label>
...
<?php echo htmlentities($announce['description']); ?>
    ...
    <div class='form-group'>
		<button type="submit" class="btn btn-primary" name="submit" id="btnSubmit">Edit Announcement</button>
		<button type="submit" class="btn btn-primary" name="delete">Delete Announcement</button>
	</div>
